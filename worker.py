...
from pcode.utils.class_balanced import compute_class_weights, inverse_frequency_loss, evaluate_class_accuracy, weights_decay
...
class Worker(object):
    def __init__(self, conf):
        ...
        self.use_class_accuracy = conf.use_class_accuracy
    ...
    def _recv_model_and_weight_from_master(self):
        """Receive the model and accuracy from the master."""
        old_buffer = copy.deepcopy(self.model_tb.buffer)
        combined_buffer = torch.zeros(self.model_tb.buffer.numel() + 1, dtype=torch.float32)
        dist.recv(tensor=combined_buffer, src=0)
        
        # Separate the model and accuracy from the combined buffer
        model_buffer = combined_buffer[:-1]
        received_weight = combined_buffer[-1].item()
        
        # Load the model from the buffer
        self.model_tb.buffer.copy_(model_buffer)
        self.model_tb.unpack(self.model_state_dict.values())
        self.model.load_state_dict(self.model_state_dict)
        
        # Store the received accuracy
        self.decay_weight = received_weight
        
        random_reinit.random_reinit_model(self.conf, self.model)
        self.init_model = self._turn_off_grad(copy.deepcopy(self.model).to(self.device))
        
        self.conf.logger.log(
            f"Worker-{self.conf.graph.worker_id} (client-{self.conf.graph.client_id}) received the model ({self.arch}) and accuracy from Master.\n        The model status {'is updated' if old_buffer.norm() != self.model_tb.buffer.norm() else 'is not updated'}.\n        Received weight={self.decay_weight}."
        )
        
        dist.barrier()
        
    def _recv_model_and_group_from_master(self):
        """Receive the model and group index from the master."""
        old_buffer = copy.deepcopy(self.model_tb.buffer)
        combined_buffer = torch.zeros(self.model_tb.buffer.numel() + 1, dtype=torch.float32)
        dist.recv(tensor=combined_buffer, src=0)

        # Separate the model and group index from the combined buffer
        model_buffer = combined_buffer[:-1]
        received_group_index = int(combined_buffer[-1].item())
        ...
        # Store the received group index
        self.group_index = received_group_index

        random_reinit.random_reinit_model(self.conf, self.model)
        self.init_model = self._turn_off_grad(copy.deepcopy(self.model).to(self.device))

        self.conf.logger.log(
            f"Worker-{self.conf.graph.worker_id} (client-{self.conf.graph.client_id}) received the model ({self.arch}) and group index from Master.\n        The model status {'is updated' if old_buffer.norm() != self.model_tb.buffer.norm() else 'is not updated'}.\n        Received group index={self.group_index}."
        )
    ...
    def _train(self):
        ...
        self.scheduler = create_scheduler.Scheduler(self.conf, optimizer=self.optimizer)
        self.tracker = RuntimeTracker(conf=self.conf, metrics_to_track=self.metrics.metric_names, use_class_accuracy=self.use_class_accuracy)
        self.conf.logger.log(
            f"Worker-{self.conf.graph.worker_id} (client-{self.conf.graph.client_id}) enters the local training phase (current communication rounds={self.conf.graph.comm_round})."
        )
    ...
    def _inference(self, data_batch):
        ...
        if self.conf.data in ['sst', 'ag_news']:
            ...
            output = self.model(data_batch["input"])
        # Compute the loss and performance
        class_correct = None
        class_total = None
        if self.conf.use_mixup:
            ...
            # Evaluate performance for both mixup targets
            performance_a, class_correct_a, class_total_a = self.metrics.evaluate(
                loss, output, data_batch["target_a"], use_class_accuracy=self.tracker.use_class_accuracy
            )
            performance_b, class_correct_b, class_total_b = self.metrics.evaluate(
                loss, output, data_batch["target_b"], use_class_accuracy=self.tracker.use_class_accuracy
            )
            
            # Weighted average of the two performances
            performance = [
                data_batch["mixup_lambda"] * _a + (1 - data_batch["mixup_lambda"]) * _b
                for _a, _b in zip(performance_a, performance_b)
            ]
            if self.tracker.use_class_accuracy:
                class_correct = {
                    cls: data_batch["mixup_lambda"] * class_correct_a.get(cls, 0)
                         + (1 - data_batch["mixup_lambda"]) * class_correct_b.get(cls, 0)
                    for cls in set(class_correct_a.keys()).union(class_correct_b.keys())
                }
                class_total = {
                    cls: class_total_a.get(cls, 0) + class_total_b.get(cls, 0)
                    for cls in set(class_total_a.keys()).union(class_total_b.keys())
                }
        else:
