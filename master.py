...
class Master(object):
    ...
    def run(self):
        for comm_round in range(1, 1 + self.conf.n_comm_rounds):
            ...
            flatten_local_models = self._receive_models_from_selected_clients(selected_client_ids)
    ...
    def _receive_models_accuracies_from_selected_clients(self, selected_client_ids):
        self.conf.logger.log(f"Master waits to receive the local models.")
        dist.barrier()
        
        # Initialize placeholders for receiving local models and accuracies.
        flatten_local_models = dict()
        local_top1_accuracies = dict()
        reqs = []
        ...
        for selected_client_id in selected_client_ids:
            ...
            combined_buffer_size = client_tb.buffer.numel() + 1
        ...
        for client_id in selected_client_ids:
            ...
            model_buffer = combined_buffer[:-1]  # Model parameters
            top1_accuracy = combined_buffer[-1].item()  # Top1 accuracy
            ...
            local_top1_accuracies[client_id] = top1_accuracy
        
        return flatten_local_models, local_top1_accuracies
    ...
    def _avg_over_archs(self, flatten_local_models):
        ...
        for arch in archs:
            ...
            self.conf.logger.log(
                f"Master received {len(_flatten_local_models)} models ({arch})."
            )
    ...
    def _aggregate_model_and_evaluate(self, flatten_local_models):
        ...
        # If all models in flatten_local_models have the same architecture, we only need to take the first aggregated model, otherwise None
        # _avg_over_archs returns a dictionary {architecture: aggregated model}
        ...
        if self.aggregator.aggregate_fn is not None:
            ...
            if fedavg_model is not None:  # Only one architecture type
                # Calculate average performance across multiple data loaders
            ...
            else:  # Evaluate different aggregated models for different architectures in the aggregation scheme
            ...
            # Further aggregate models based on the aggregator's aggregation function and conditions. If the aggregation function exists, model aggregation will be performed according to the function. This is to perform further aggregation operations on aggregated models and local models of different architectures based on specific aggregation strategies and conditions. This is to meet more advanced aggregation strategies and requirements for better integration and model updates.
            client_models = self.aggregator.aggregate(
                master_model=self.master_model,
                client_models=self.client_models,
                fedavg_model=fedavg_model,
                fedavg_models=fedavg_models,  # Dictionary {architecture: aggregated model}
                flatten_local_models=flatten_local_models,
                performance=performance,
            )
            # In the case of the same model architecture, client_models and self.master_model will be updated in-place.
        ...
        # If the aggregation strategy is federated_average
        else:
